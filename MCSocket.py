...
if(len(dict) != 0):
    ...
    with open("requests.json", 'w') as json_file:
            dump(dict, json_file, indent=4, sort_keys=True)
else:
...
async def mineproxy(websocket):
    ...
    try: 
        ...
        await websocket.send(dumps(format_json("kill @e[type=villager]")))
        ...
        for value in v1_bin:
            if value == 1:
                await websocket.send(dumps(format_json(f"summon villager {coords_input[value_check]}")))
            value_check+= 1
        value_check = 3
        for value in v2_bin:
            if value == 1:
        ...
        if v1_neg == False:
            await websocket.send(dumps(format_json("summon villager -29 -52 9")))
        if v2_neg == False:
            await websocket.send(dumps(format_json("summon villager -23 -37 21")))
        ...
        if op == "x":
            ...
            for coords in ["-13 -59 81", "-12 -59 81", "-11 -59 81", "-10 -59 81", "-9 -59 81", "-8 -59 81"]:
                await websocket.send(dumps(format_json(f"testforblock {coords} lit_redstone_lamp")))
            #AJOUTER LE GETTER DE BIT DE POIDS FORT
        elif op == "+":
            sleep(10)
            for coords in ["-12 -59 50", "-11 -59 50", "-10 -59 50", "-9 -59 50"]:
                await websocket.send(dumps(format_json(f"testforblock {coords} lit_redstone_lamp")))
            await websocket.send(dumps(format_json(f"testforblock -8 -59 50 lit_redstone_lamp")))
...
async def main():
    async with serve(mineproxy, host='localhost', port=3000):
